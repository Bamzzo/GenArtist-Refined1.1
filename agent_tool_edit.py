...
def main_edit(args):
    ...
    if args["tool"] == "addition_anydoor":
        os.sys.path.insert(0, str(_PROJECT_ROOT / "AnyDoor"))
        os.sys.path.insert(0, str(_PROJECT_ROOT / "AnyDoor" / "dinov2"))
        os.sys.path.insert(0, str(_PROJECT_ROOT / "AnyDoor" / "dinov2" / "datasets"))
        ...
        sd2_path = str(MODEL_ZOO_DIR / "stable-diffusion-2")
        pipe = StableDiffusionDiffEditPipeline.from_pretrained(
            sd2_path, torch_dtype=torch.float16, use_safetensors=True
        )
        pipe = pipe.to("cuda")
        ...
        os.sys.path.insert(0, str(_PROJECT_ROOT / "Inpaint-Anything"))
        ...
        lama_config = str(_PROJECT_ROOT / "Inpaint-Anything" / "lama" / "configs" / "prediction" / "default.yaml")
        lama_ckpt = str(MODEL_ZOO_DIR / "LaMa" / "big-lama")
        img_inpainted = inpaint_img_with_lama(
            img, masks, lama_config, lama_ckpt, device='cuda')
        ...
        os.sys.path.insert(0, str(_PROJECT_ROOT / "instruct-pix2pix"))
        from edit_cli import CFGDenoiser, load_model_from_config

        sys.path.insert(0, str(_PROJECT_ROOT / "instruct-pix2pix" / "stable_diffusion"))
        step = 50
        config = OmegaConf.load(str(_PROJECT_ROOT / "instruct-pix2pix" / "configs" / "generate.yaml"))
        magicbrush_ckpt = str(MODEL_ZOO_DIR / "MagicBrush" / "MagicBrush-epoch-000168.ckpt")
        # model = load_model_from_config(config, str(MODEL_ZOO_DIR / "MagicBrush" / "MagicBrush-epoch-52-step-4999.ckpt"), None)
        model = load_model_from_config(config, magicbrush_ckpt, None)
...
def run_edit_tool(command_dict: dict) -> str | None:
    """
    执行编辑类工具（addition_anydoor / replace_anydoor / remove / instruction / attribute_diffedit）。
    command_dict 格式与原 input.json 一致，需包含 "tool", "input", "output" 等。
    返回写入的图片路径，失败或无输出时返回 None。
    """
    return main_edit(command_dict)


if __name__ == "__main__":
    _cmd = {
        "tool": "instruction",
        "output": "inputs/368667-input-output.png",
        "input": {"image": "inputs/368667-input.png", "text": "The farm should have a stream, and a giraffe should be on the left of the stream under a supernova explosion sky."},
    }
    out = run_edit_tool(_cmd)
