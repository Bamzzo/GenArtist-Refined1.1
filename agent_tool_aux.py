...
def main_aux(args):
    ...
    if args["tool"] == "object_addition_anydoor":
        ...
        sam_path = str(MODEL_ZOO_DIR / "sam-vit-base")
        model = SamModel.from_pretrained(sam_path, torch_dtype=torch.float16, use_safetensors=True).to("cuda")
        processor = SamProcessor.from_pretrained(sam_path)
        raw_image = output
        input_boxes = [[layout]]
        ...
        result = {"output": args["output"], "output_mask": args["output_mask"]}
        ...
        model = load_model(
            ...
            str(MODEL_ZOO_DIR / "GroundingDINO" / "weights" / "groundingdino_swint_ogc.pth"),
        )
...
def run_aux_tool(command_dict: dict) -> dict | None:
    """
    执行辅助类工具（object_addition_anydoor / detection / segmentation）。
    command_dict 格式与原 input.json 一致。
    返回: detection 时为 {"detection": [(phrase, bbox), ...]}；
          object_addition_anydoor 为 {"output": path, "output_mask": path}；
          segmentation 为 {"output": path}。未匹配或异常时返回 None。
    """
    return main_aux(command_dict)


if __name__ == "__main__":
    _cmd = {"tool": "detection", "input": {"image": "./inputs/0.png", "text": "car . person"}}
    out = run_aux_tool(_cmd)
    print("Result:", out)
